...
def getNearestTaxies(request):
    if 'latitud' in request.GET:
        ...
        if customer.sessionID != request.GET['sessionID']:
            return HttpResponse(status=status.HTTP_401_UNAUTHORIZED, content="Debes estar conectado para realizar esta acción")
        #Get a list with closest drivers meeting user filters
        closestDrivers = Driver.objects.filter(car__isfree=True, available=True, car__accessible__in=[customer.fAccessible, True], car__animals__in=[customer.fAnimals, True], car__appPayment__in=[customer.fAppPayment, True], car__capacity__gte=customer.fCapacity).distance(pointclient).order_by('distance')[:10]
...
def loadData(request):
    ...
    car.save()
    dr = Driver(email="conductor@gmail.com", password="71c81ab34424bdcefcc22d37effa65ce161828a41f0d0c61ba8ac647c6642487", phone="+34656111112", first_name="Conductor", last_name="DePrimera", city = City.objects.get(id=34), validationCode=1234, positiveVotes=10, negativeVotes=3, car=car, geom=Point(43.2618425, -2.9327811), isValidated=True, image="", pushID='APA91bHJRkpSjXvlFA7L94ybyalAeW0BxE0Z1K4g99onHvXLIFgptSJDhBIMXckY9HBzaBpEWo4Se9zUCd2KjzWUHCJ5TLac-qF-Hu8ozi7Uoe14ZFRg2_c82xmL4ZXgMfuhec4UUd-eu_SkYsMPRt2bqNZ0K5Uzgpwd2en9454w8-f3c7pyEK0')
    dr.save()
    ...
    car.save()
    dr2 = Driver(email="conductor2@gmail.com", password="71c81ab34424bdcefcc22d37effa65ce161828a41f0d0c61ba8ac647c6642487", phone="+34656111113", first_name="Conductor", last_name="DeSegunda", city = City.objects.get(id=54), validationCode=1234, positiveVotes=2, negativeVotes=7, car=car, geom=Point(43.264116, -2.9237662), isValidated=True, image="", pushID='APA91bHTypZCKvUdXYd-lhimPaLbolkEvZU8o9o5FWhRW0tIx5JpcIS3mdNYza0o5F0d-lBzn3xYw2RBZWfJEy_wdOLIZVwefcUsRtG_PpGXyauJ0EnnOND-zS0dOOAcb_xG2QhqodKQchzJgV6-z41y8zsPwMzJrNY2Bj-kCeUsm-Ca3kKH0j4')
    dr2.save()
    cu = Customer(email="gorka_12@hotmail.com", password="71c81ab34424bdcefcc22d37effa65ce161828a41f0d0c61ba8ac647c6642487", phone="+34656111111", first_name="Pepito", last_name="Palotes", city = City.objects.get(id=20), validationCode=1234, lastUpdate=datetime.strptime('1980-01-01 00:00:01','%Y-%m-%d %H:%M:%S'), isValidated=True, image="", pushID='APA91bHTypZCKvUdXYd-lhimPaLbolkEvZU8o9o5FWhRW0tIx5JpcIS3mdNYza0o5F0d-lBzn3xYw2RBZWfJEy_wdOLIZVwefcUsRtG_PpGXyauJ0EnnOND-zS0dOOAcb_xG2QhqodKQchzJgV6-z41y8zsPwMzJrNY2Bj-kCeUsm-Ca3kKH0j4')
    cu.save()
    cu.favlist.add(dr)
    cu.favlist.add(dr2)
...
def loadTravels(request):
    cu1 = Customer(email="laura@gmail.com", password="71c81ab34424bdcefcc22d37effa65ce161828a41f0d0c61ba8ac647c6642487", phone="+34656222222", first_name="laura", last_name="linacero", city = City.objects.get(id=56), validationCode=1234, lastUpdate=datetime.strptime('1980-01-01 00:00:01','%Y-%m-%d %H:%M:%S'), isValidated=True, image="", pushID='APA91bHTypZCKvUdXYd-lhimPaLbolkEvZU8o9o5FWhRW0tIx5JpcIS3mdNYza0o5F0d-lBzn3xYw2RBZWfJEy_wdOLIZVwefcUsRtG_PpGXyauJ0EnnOND-zS0dOOAcb_xG2QhqodKQchzJgV6-z41y8zsPwMzJrNY2Bj-kCeUsm-Ca3kKH0j4')
    cu1.save()
