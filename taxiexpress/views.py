...
def getClosestTaxi(request):
    if 'latitude' in request.POST:
        ...
        post_data = {"title" : 'Viaje disponible' , "message" : 801, "origin": request.POST['origin'], "startpoint": "bb", "travelID": request.POST['travelID'], "email": request.POST['email'], "phone": request.POST['phone']}     
...
def getSelectedTaxi(request):
    if 'sessionID' in request.POST:
        ...
        post_data = {"pushId": driver.pushID , "title" : "Viaje disponible" , "message" : 802 , "startpoint": "a", "travelID": travel.id, "customerID": customer.id, "phone": customer.phone} 
        try:
